...
function SnowflakeService(connectionConfig, httpClient, config)
{
  ...
  var currentState;
  var isStageCreated = false;

  var logFunction = connectionConfig.logFunction;
  ...
  * @inheritDoc
  */
  OperationRequest.prototype.executeAsync = function ()
  {
    return currentState.requestAsync(this.options);
  };
...
function StateAbstract(options)
{
  ...
  /**
  * Sends out the post request.
  *
  * @returns {Object} the request that was issued.
  */
  Request.prototype.sendAsync = function ()
  {
    // pre-process the request options
    this.preprocessOptions(this.requestOptions);

    var url = this.requestOptions.absoluteUrl;
    var header = this.requestOptions.headers;
    var body = this.requestOptions.json;
...
StateConnecting.prototype.continue = function ()
{
  ...
  var err = context.options.err;
  var json = context.options.json;
    ...
    else
    {
      if (Errors.isNetworkError(err) || Errors.isRequestFailedError(err))
      {
        ...
        else
        {
          ...
          connectionConfig.coalesceLog("Ran out of retries, transitioning to disconnected", err);
          // we're now disconnected
          parent.snowflakeService.transitionToDisconnected(err);
        }
      }
      else
      {
        connectionConfig.coalesceLog("Non-retryable error, transitioning to disconnected", err);
        // we're now disconnected
        parent.snowflakeService.transitionToDisconnected(err);
      }
    }
...
StateConnected.prototype.requestAsync = function (options)
{
  ...
  options.callback = async function (err, body)
  {
    ...
    else
    {
        ...
        (err.code === GSErrors.code.GONE_SESSION))
      {
        ...
        // we're disconnected
        this.connectionConfig.coalesceLog("Invalid/missing session", err);
        this.snowflakeService.transitionToDisconnected(err);
  ...
  this.createSessionTokenRequest(
    {
      ...
      callback: function (err, body)
      {
        ...
        if (!err || err.code === GSErrors.code.GONE_SESSION)
        {
          ...
          err = undefined;
          this.connectionConfig.coalesceLog("Disconnect complete", err);
          this.snowflakeService.transitionToDisconnected(originalError);
        }
  ...
  this.createMasterTokenRequest(
    {
      ...
      callback: function (err, body)
      {
        ...
        else
        {
          ...
          {
            this.connectionConfig.coalesceLog("During master token request, got MASTER_TOKEN_EXPIRED", err);
            this.snowflakeService.transitionToDisconnected(err);
          }
               ...
               // disconnected
          {
            ...
            this.connectionConfig.coalesceLog("During master token request, request failed for an unexpected reason!", err);
            this.snowflakeService.transitionToDisconnected(err);
          }
        }
...
StateDisconnected.prototype.connect = function (options)
{
  ...
  process.nextTick(function ()
  {
    callback(Errors.createClientErrorWithCause(
      ErrorCodes.ERR_CONN_CONNECT_STATUS_DISCONNECTED), undefined, this?.cause);
  });
};
...
StateDisconnected.prototype.request = function (options)
{
  ...
  {
    callback(Errors.createClientErrorWithCause(
      ErrorCodes.ERR_CONN_REQUEST_STATUS_DISCONNECTED, true, this?.cause));
  });
};
...
StateDisconnected.prototype.destroy = function (options)
{
  ...
  {
    callback(Errors.createClientErrorWithCause(
      ErrorCodes.ERR_CONN_DESTROY_STATUS_DISCONNECTED), undefined, this?.cause);
  });
};
 ...
 * @inheritdoc
 */
StateDisconnected.prototype.enter = function (cause)
{
  this.cause = cause;
}
 ...
 * @inheritdoc
 */
StateDisconnected.prototype.exit = function ()
{
  this.cause = null;
}
