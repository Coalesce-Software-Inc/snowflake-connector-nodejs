...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo)
{
  ...
  var logFunction = (...args) => {};
  if (Util.exists(options.logFunction)) {
    Errors.checkArgumentValid(Util.isFunction(options.logFunction),
    ErrorCodes.ERR_CONN_CREATE_INVALID_LOG_FUNCTION_COALESCE);
...
ConnectionConfig.prototype.coalesceLog = function(message, error, ...args)
{
  const errorArgs = [];
  if (error) {
    const responseBody = error.responseBody ?? error.response?.body ?? undefined;
    const responseHttpCode = error.response?.code;
    errorArgs.push(
      `name: ${error.name}`, `code: ${error.code}`, `message: ${error.message}`,
      ...(responseBody ? ["responseBody: ", responseBody] : []),
      responseHttpCode ? `responseHttpCode: ${responseHttpCode}` : '',
      ...(error.cause ? ["cause=", error.cause] : []),
    )
  }
  // log everything combined
  this.logFunction(message, ...errorArgs, ...args);
}
